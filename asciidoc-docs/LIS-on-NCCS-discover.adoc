...
//TODO: MobaXTerm setup instructions
...
module use --append $HOME/{module-dir}
...
----
+
This file will be executed every time you log onto Discover to set up your environment.

. Next, you'll need a convenient way to load environment variables that LISF components require to compile and run. Discover provides this functionality through the program link:https://www.nccs.nasa.gov/nccs-users/instructional/using-discover/miscellaneous/using-modules[modules].
+
LISF-specific module files for use on Discover are included alongside the LISF source code in the link:{url-lisf-github}/tree/master/env/discover[`env/discover` directory] of the GitHub repository. In this step we will download the LISF modulefile for the SLES12 environment to Discover.
+
First, make a directory called `{module-dir}/` in your `$HOME` directory to store the module files and change into it:
+
[source,bash,subs="attributes"]
----
% mkdir {module-dir}
...
----
+
Using `curl`, download the latest LISF modulefile into `{module-dir}/`:
+
[source,bash,subs="attributes"]
----
